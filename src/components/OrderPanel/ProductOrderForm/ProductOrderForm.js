...
const renderForm = formRenderProps => {
  ...
  const { planName, isActive } = currentPlanData || {};
  ...
  const buttonDisabledFreePlan =
    (planName === 'Free-Plan' && listingsSoldCount > 50) || listingsSoldPrice > 500;
  const buttonDisabledBusinessMember =
    (planName === 'business-enterprise' && listingsSoldCount > 1000) || listingsSoldPrice > 25000;
  ...
  return (
    <Form onSubmit={handleFormSubmit}>
      ...
      <span className={css.points}>
        <IconCollection icon="pointsIcon" />
        <FormattedMessage id="ProductOrderForm.pointsText" />
      </span>

      <div className={css.submitButton}>
        {!buttonDisabledFreePlan && !buttonDisabledBusinessMember ? (
          <PrimaryButton
            type="submit"
