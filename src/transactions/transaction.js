...
import * as cryptoProcess from './transactionProcessCrypto';
...
export const CRYPTO_PROCESS_NAME = 'crypto-purchase';
/**
...
const PROCESSES = [
  ...
  {
    name: CRYPTO_PROCESS_NAME,
    alias: `${CRYPTO_PROCESS_NAME}/release-1`,
...
export const resolveLatestProcessName = processName => {
  switch (processName) {
    ...
    case BOOKING_PROCESS_NAME:
      ...
      case CRYPTO_PROCESS_NAME:
      return CRYPTO_PROCESS_NAME;
...
/**
 * Check if the process is crypto process
 *
 * @param {String} processName
 */

export const iscryptoProcess = processName => {
  const latestProcessName = resolveLatestProcessName(processName);
  const processInfo = PROCESSES.find(process => process.name === latestProcessName);
  return [CRYPTO_PROCESS_NAME].includes(processInfo?.name);
};

/**
 * Check if the process/alias points to a crypto process
 *
 * @param {String} processAlias
 */
export const iscryptoProcessAlias = processAlias => {
  const processName = processAlias ? processAlias.split('/')[0] : null;
  return processAlias ? iscryptoProcess(processName) : false;
};
