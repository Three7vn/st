...
import { listingCategories, listingSubCategories } from '../../config/configListing';
...
export const ListingPageComponent = props => {
  ...
  const { formattedPrice } = (price, config.currency, intl);
  ...
  const handleSubmitOffer = values => {
    const { offerPrice, listingPrice } = values;
    const initialValues = {
      listing,
      orderData: {
        offerPrice,
  ...
  const index = cartItems && cartItems.findIndex(e => e == currentListing.id.uuid);
    ...
    try {
      if (!currentUser) {
        ...
        if (index > -1) {
          ...
          const addedcartItems = Array.from(new Set(cartItems));
          const profile = {
            protectedData: {
          ...
          cartItems.push(currentListing.id.uuid);
          const addedcartItems = Array.from(new Set(cartItems));
          const profile = {
            protectedData: {
              cartItems: addedcartItems,
  ...
  const makeAnOfferModal = (
    <Modal
      ...
      onClose={() => setisMakeAnOfferModal(false)}
      usePortal
      onManageDisableScrolling={onManageDisableScrolling}
    ...
    >
      <MakeAnOfferForm
        ...
        onSubmit={values => handleSubmitOffer(values)}
        inProgress={sendInquiryInProgress}
        config={config}
      />
    </Modal>
        ...
        <div className={css.contentWrapperForProductLayout}>
          <div className={css.mainColumnForProductLayout}>
            <div className={css.breadCrumb}>
              ...
              <span
                onClick={() => {
                  history.push(`/s?refinementList=${category}`);
                }}
              >
                {getLabel(listingCategories, category)}/
              </span>
              <span
                onClick={() => {
          ...
          <div className={css.orderColumnForProductLayout}>
            <OrderPanel
              ...
              handleMakeAnOfferModal={() => setisMakeAnOfferModal(true)}
              index={index}
              reviews={reviews}
              currentUser={currentUser}
              onSubmit={handleOrderSubmit}
              handleAuthorModal={() => setInquiryModalOpen(true)}
              authorLink={
                <NamedLink
...
const mapDispatchToProps = dispatch => ({
  ...
  onSendOffer: (listing, data) => dispatch(sendOffer(listing, data)),
});
