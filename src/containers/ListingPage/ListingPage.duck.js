...
import { transactionLineItems, updateTransactionMetadata } from '../../util/api';
...
const listingPageReducer = (state = initialState, action = {}) => {
  ...
  switch (type) {
    ...
    case AUTHOR_LISTINGS_SUCCESS:
      return { ...state, authorListings: payload };
...
export const fetchAuthorListings = ({ authorId }) => async (dispatch, getState, sdk) => {
  try {
    ...
    if (result) {
      dispatch(authorListingsSuccess(result?.data?.data?.length));
    }
    return result;
  } catch (error) {}
};
...
export const showListing = (listingId, config, isOwn = false) => (dispatch, getState, sdk) => {
  ...
  return show
    .then(data => {
      ...
      // dispatch(transactionCount({ authorId }));
...
export const sendInquiry = (listing, message) => (dispatch, getState, sdk) => {
  ...
  const processAlias = 'default-inquiry/release-1';
  if (!processAlias) {
...
export const transactionCount = ({ authorId }) => (dispatch, getState, sdk) => {
  return sdk.transactions
    .query({
      userId: new UUID(authorId),
      only: 'sale',
      lastTransitions: ['transition/review-2-by-provider'],
    })
    .then(res => {
...
export const sendOffer = (listing, data) => (dispatch, getState, sdk) => {
  dispatch(sendInquiryRequest());
  ...
  const { listingPrice, offerPrice } = data || {};
  const bodyParams = {
...
export const loadData = (params, search, config) => dispatch => {
  ...
  return Promise.all([
    ...
    // dispatch(transactionCount(listingId)),
  ]).then(response => {
