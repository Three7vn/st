...
export const AuthenticationPageComponent = props => {
  ...
  const {subscriptionPlan} = !!user?.id && user.attributes.profile.protectedData || {};
  const {freePlanData} = !!user?.id && user.attributes.profile.publicData || {};

  // Already authenticated, redirect away from auth page
  if (isAuthenticated && from) {
    return <Redirect to={from} />;
  } else if (isAuthenticated && currentUserLoaded && !showEmailVerification && (subscriptionPlan?.length > 0 ||freePlanData?.isFreePlanActive)) {
    return <NamedRedirect name="LandingPage" />;
  } else if (isAuthenticated && currentUserLoaded && !showEmailVerification && !(subscriptionPlan?.length || freePlanData?.isFreePlanActive)) {
    return <NamedRedirect name="StripeSubscriptionPage" />;
  }
