...
import { fetchRedeemedVouchers, onApplyDiscountCoupoun } from '../../../util/api';
import { useDispatch, useSelector } from 'react-redux';
import { redeemedVoucherSuccess } from '../CheckoutPage.duck';
...
const LocationOrShippingDetails = props => {
  ...
  return askShippingDetails ? (
    <ShippingDetails
      ...
      onRedeemedVouchers={onRedeemedVouchers}
      deliveryMethodMaybe={deliveryMethodMaybe}
...
const initialState = {
  ...
  paymentMethod: null,
  selectPaymentMode: PAYOUT_WITH_STRIPE,
};
...
class StripePaymentForm extends Component {
  ...
  handleSubmit(values) {
    const {
      ...
      hasHandledCardPayment,
      defaultPaymentMethod,
      handleCryptoPayments,
    } = this.props;
    const { initialMessage } = values;
    const { cardValueValid, paymentMethod, selectPaymentMode } = this.state;
    ...
    if ((inProgress || onetimePaymentNeedsAttention) && selectPaymentMode === PAYOUT_WITH_STRIPE) {
    ...
    selectPaymentMode === PAYOUT_WITH_CRYPTO ? handleCryptoPayments(values) : onSubmit(params);
  }
  ...
  paymentForm(formRenderProps) {
    ...
    const submitDisabled = invalid || submitInProgress || !selectedRateState;
    ...
    useEffect(() => {
      ...
      script.src = 'https://cdn.triple-a.io/widgets/triplea-ecommerce-payment-v1.1/js/app.js';
      script.async = true;
    ...
    const offerPrices = pageData?.orderData?.offerPrice?.amount / 100 || {};
    const planName =
      pageData?.listing?.author?.attributes?.profile?.publicData?.currentPlanData?.planName || {};
    return hasStripeKey ? (
      <Form className={classes} onSubmit={handleSubmit} enforcePagePreloadFor="OrderDetailsPage">
        <div>
          <h4 className={css.subTitle}>select one payment method</h4>
          <div className={css.buttonsGroup}>
            <span
              onClick={() => {
                this.setState({ selectPaymentMode: PAYOUT_WITH_STRIPE });
              }}
            ...
            <span
              onClick={() => {
                this.setState({ selectPaymentMode: PAYOUT_WITH_CRYPTO });
              }}
              className={css.button}
            >
        ...
        {selectPaymentMode === PAYOUT_WITH_CRYPTO ? (
          <div>
            <Modal
            ...
            >
              {paymentModal && (
                <triplea-ecommerce-payment-v1
                  payment-reference={paymentDetails}
        ...
        {selectPaymentMode === PAYOUT_WITH_STRIPE ? (
          <>
            {billingDetailsNeeded && !loadingData ? (
              ...
              <p className={css.spinner}>{/* <IconSpinner /> */}</p>
            ) : null}
          </>
        ) : null}
