...
export const processCheckoutWithPayment = (orderParams, extraPaymentParams) => {
  ...
  const offerPrice = pageData?.orderData?.offerPrice?.amount / 100 || null;
  ...
  const fnRequestPayment = fnParams => {
    ...
    const orderPromise = hasPaymentIntents
      ...
      : offerPrice
      ? onMakeAnOffer(fnParams, processAlias, offerPrice, transitions, planName).then(res => {
          if (res) {
            const order = res || {};
            ...
            return hasPaymentIntentUserActionsDone
              ...
              : onConfirmCardPayment(params).then(res => {
                  onSavePaymentMethod(ensuredStripeCustomer, res?.paymentIntent?.payment_method);
                  history.push(
                    createResourceLocatorString(
                      'OrderDetailsPage',
                      routeConfiguration,
                      { id: res?.transactionId?.uuid },
                      {}
                    )
                  );
                });
