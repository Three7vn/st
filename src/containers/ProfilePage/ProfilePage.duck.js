...
import { fetchFollowersData, followUserStore } from '../../util/api';
...
export default function profilePageReducer(state = initialState, action = {}) {
  ...
  switch (type) {
    ...
    case QUERY_REVIEWS_SUCCESS:
      let updatedRreviews = [...(state?.reviews || [])];
      const index = updatedRreviews.findIndex(elm => elm.userId === payload.userId);

      if (index > -1) {
        updatedRreviews[index] = { ...updatedRreviews[index], ...payload };
      } else {
        updatedRreviews.push(payload);
      }

      return { ...state, reviews: updatedRreviews };
...
export const queryUserReviews = userId => (dispatch, getState, sdk) => {
  sdk.reviews
    ...
    .then(response => {
      ...
      dispatch(queryReviewsSuccess({ reviews, userId }));
    })
...
export const followStore = params => async (dispatch, getState, sdk) => {
  try {
    const response = await followUserStore({ ...params });
    return response;
  } catch (error) {
...
export const fetchFollowUser = params => async (dispatch, getState, sdk) => {
  try {
    const response = await fetchFollowersData(params);
    return response;
  } catch (error) {
    console.log(error, 'error');
  }
