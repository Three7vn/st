...
const GroupOfFieldCheckboxes = props => {
  const { id, className, name, options, urlQueryParams } = props;
  ...
  const uniqueArray = () => {
    if (name === 'subcategory') {
      ...
      if (urlQueryParams?.pub_category) {
        filteredOptions = options.filter(e => urlQueryParams?.pub_category.split(',').includes(e.parentKey));
      }

      // Use Set to eliminate duplicates based on the key
      const uniqueSet = new Set(filteredOptions.map(e => e.key));

      // Map the unique keys back to their corresponding objects
      return Array.from(uniqueSet, key => filteredOptions.find(e => e.key === key));
    }

    if (name === 'subcategorychild') {
      let filteredOptions = options;
      if (urlQueryParams?.pub_subCategory) {
        filteredOptions = options.filter(e => urlQueryParams?.pub_subCategory.split(',').includes(e.parentSubCategory));
      }
    ...
    // If no specific conditions match, return the original array
    return options;
  };

  const filterOptions = uniqueArray();
  return (
