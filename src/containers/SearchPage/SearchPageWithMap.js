...
import { Button, H3, H5, ModalInMobile, Page } from '../../components';
...
export class SearchPageComponent extends Component {
  ...
  render() {
    ...
    const { keywords, address } = parse(location.search, {
      latlng: ['origin'],
      latlngBounds: ['bounds'],
    });
    ...
    const handleSubmit = values => {
      ...
      let validQueryParamsTemp = { ...validQueryParams }
      delete validQueryParamsTemp.keywords;
      const { history, config, routeConfiguration } = this.props;

      const topbarSearchParams = () => {
        if (isMainSearchTypeKeywords(config)) {
          return { keywords: values?.keywords, ...validQueryParamsTemp };
        }
        ...
        <div className={css.container}>
          <div className={css.searchResultContainer}>
            <div className={css.filterWrapper}>
              ...
              >
                {availableFilters.map(config => {
                  return ((validQueryParams['pub_category'] && validQueryParams['pub_subCategory']) && config.key == "subCategoryChild"
                    ? null
                    : <FilterComponent
              ...
              <MainPanelHeader
                ...
                sortByComponent={this.state.isMap ? sortBy('desktop') : null}
                isSortByActive={this.state.isMap ? sortConfig.active : null}
                listingsAreLoaded={listingsAreLoaded}
                resultsCount={totalItems}
                searchInProgress={searchInProgress}
                searchListingsError={searchListingsError}
              ...
              >
                <SearchFiltersPrimary {...propsForSecondaryFiltersToggle}>
                  {availablePrimaryFilters.map(config => {
                    return ((!validQueryParams['pub_category'] && !validQueryParams['pub_subCategory'] && !validQueryParams['pub_subCategoryChild']) && config.key == "subCategoryChild"
                      ? null
                      : <FilterComponent
                  ...
                  {!this.state.isMap ? sortBy('desktop') : null}
            ...
            {isSecondaryFiltersOpen ? (
              ...
              <div
                className={classNames(this.state.isMap ? css.listingsForMapVariant : css.listingsForGridVariant, {
