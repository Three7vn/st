...
export const EditListingPageComponent = props => {
  ...
  const {publicData,protectedData} = !!currentUser?.id && currentUser.attributes.profile || {};
  ...
  if (shouldRedirect) {
    ...
    const title = isNewListingFlow
      ...
      if(!(publicData?.freePlanData?.isFreePlanActive || protectedData?.subscriptionPlan?.length > 0)){
        return <NamedRedirect name="StripeSubscriptionPage" />;
      }

    return (
      <Page title={title} scrollingDisabled={scrollingDisabled} className={css.pageMain}>
        <TopbarContainer
