...
import { createAlgoliaData, getCarrierAccounts, updateAlgoliaData } from '../../util/api';
...
const capitalizeEachWord = text => {
  ...
  // Replace hyphens with spaces and split the string into an array of words
  const words = text.replace(/-/g, ' ').split(' ');

  // Capitalize the first letter of each word and join them back
  const capitalizedWords = words.map(word => word.charAt(0).toUpperCase() + word.slice(1));
  console.log(capitalizedWords.join(' '), "capitalizedWords.join(' ')");
    ...
    return updateStockOfListingMaybe(id, stockUpdate, dispatch)
      ...
      .then(response => {
        ...
        updateAlgoliaData({
          ...
          subCategory: capitalizeEachWord(subCategory),
          subCategoryChild: capitalizeEachWord(subCategoryChild),
          condition: capitalizeEachWord(condition),
          capacity: capitalizeEachWord(capacity),
          storage: capitalizeEachWord(storage),
          model: capitalizeEachWord(model),
          compatiblity: capitalizeEachWord(compatiblity),
          refurbished: capitalizeEachWord(refurbished),
          brand: capitalizeEachWord(brand),
          material: capitalizeEachWord(material),
          type: capitalizeEachWord(type),
          ram: capitalizeEachWord(ram),
          operatingSystem: capitalizeEachWord(operatingSystem),
          refreshRate: capitalizeEachWord(refreshRate),
          videoInput: capitalizeEachWord(videoInput),
          displayType: capitalizeEachWord(displayType),
          connectivity: capitalizeEachWord(connectivity),
          opticalZoom: capitalizeEachWord(opticalZoom),
          maximumResolution: capitalizeEachWord(maximumResolution),
          batteryType: capitalizeEachWord(batteryType),
          displayTechnology: capitalizeEachWord(displayTechnology),
          display: capitalizeEachWord(display),
          size: capitalizeEachWord(size),
          color: capitalizeEachWord(color),
          ...rest,
          user: { firstName, lastName, email, displayName, userId: currentUser?.id?.uuid },
        });
  ...
  return sdk.ownListings
    ...
    .then(response => {
      ...
      createAlgoliaData({
        ...
        availabilityPlan,
        state,
        authorData: currentUser?.id?.uuid,
        category: capitalizeEachWord(category),
        subCategory: capitalizeEachWord(subCategory),
        subCategoryChild: capitalizeEachWord(subCategoryChild),
        condition: capitalizeEachWord(condition),
...
export const uploadImagesToCloudinary = file => async (dispatch, getState, sdk) => {
  try {
    if (file && file.name) {
      const formData = new FormData();
      formData.append('file', file);
      formData.append('folder', 'Listing Images'); // This won't directly affect the upload destination
      const { data } = await axios.post(
        `${process.env.REACT_APP_SERVER_MARKETPLACE_ROOT_URL}/api/upload-images`,
        formData,
        {
          headers: {
...
export const fetchCarrierAccounts = async () => {
  try {
