...
import { receiveOffers } from '../../../../config/configListing';
...
export const EditListingPricingAndStockFormComponent = props => (
  <FinalForm
    ...
    render={formRenderProps => {
      ...
      const priceValidators = getPriceValidators(0.5, marketplaceCurrency, intl);
      ...
      const receiveOffersValidator = intl.formatMessage({
      ...
      return (
        <Form onSubmit={handleSubmit} className={classes}>
          ...
          <FieldSelect
            className={css.input}
            id={`${formId}.receiveOffers`}
            name="receiveOffers"
            label={intl.formatMessage({ id: 'EditListingPricingAndStockForm.receiveOffersLabel' })}
            placeholder={intl.formatMessage({
              id: 'EditListingPricingAndStockForm.receiveOffersPlaceholder',
            })}
            validate={required(receiveOffersValidator)}
            onChange={e => {
              // Update isOptionSelected when an option other than "Select one..." is selected
              setIsOptionSelected(!!e);
            }}
          >
            {receiveOffers.map(type => (
