  ...
  return sdk.transactions
    ...
    .then(response => {
      ...
      const lastTransition =
        txResponse?.data?.data?.attributes?.lastTransition === 'transition/request-payment';
      const params = {
        transactionId: id,
      ...
      lastTransition &&
        txRole === 'customer' &&
        dispatch(confirmPayment(params)).then(() => {
          dispatch(fetchTransaction(id, txRole, config));
        });
      try {
...
export const makeReceivedTransition = (txId, transitionName, orderParams) => async (
  dispatch,
  getState,
  sdk
) => {
  ...
  const { sellerEmail, payoutAmount, sellerId } = orderParams;
  ...
  try {
    ...
    if (payoutResponse) {
      const response = await sdk.transactions.transition(
        {
          ...
          params: { protectedData: { payoutRefence: payoutResponse.payout_reference } },
        },
        { expand: true }
      );
      dispatch(addMarketplaceEntities(response));
      ...
      await refreshTransactionEntity(sdk, txId, dispatch);
      return response;
    }
  } catch (e) {
...
export const makeShippingLabelTransition = transitionParams => (dispatch, getState, sdk) => {
  const { txId, transitionName, ...rest } = transitionParams || {};
  if (transitionInProgress(getState())) {
    return Promise.reject(new Error('Transition already in progress'));
  }
  dispatch(transitionRequest(transitionName));
  createShippingLabel({ ...rest }).then(res => {
    if (res) {
      const { label_url, tracking_url_provider, object_id, tracking_number, tracking_status } =
        res || {};
      const labelData = {
        label_url,
...
export const makeReturnShippingLabelTransition = transitionParams => (dispatch, getState, sdk) => {
  const { txId, transitionName, ...rest } = transitionParams || {};
  if (transitionInProgress(getState())) {
    return Promise.reject(new Error('Transition already in progress'));
  }
  dispatch(transitionRequest(transitionName));
  createReturnShippingLabel({ ...rest }).then(res => {
    if (res) {
      const { label_url, tracking_url_provider, object_id, tracking_number, tracking_status } =
        res || {};
...
export const makeTransition = (
  ...
  const customerplanName =
    params?.customer?.attributes?.profile?.publicData?.freePlanData?.planName ||
    params?.customer?.attributes?.profile?.publicData?.currentPlanData?.planName;

  const providerPlanName =
    params?.provider?.attributes?.profile?.publicData?.freePlanData?.planName ||
    params?.provider?.attributes?.profile?.publicData?.currentPlanData?.planName;
  const firstTx = params?.provider?.attributes?.profile?.publicData?.firstTx || false;

  const providerUsername = params?.provider?.attributes?.profile?.publicData?.username || '';
  const customerUsername = params?.customer?.attributes?.profile?.publicData?.username || '';
  const providerEmail = params?.provider?.attributes?.profile?.publicData?.email || '';
  const customerEmail = params?.customer?.attributes?.profile?.publicData?.email || '';

  const customerUserId = params?.customer?.id?.uuid || '';
  const providerUserId = params?.provider?.id?.uuid || '';

  if (transitionName === transitions?.OFFER_ACCEPT_BY_PROVIDER) {
    ...
    // Update params with orderNumber
    params = {
      ...params,
      protectedData: {
  ...
  return sdk.transactions
    ...
    .then(async response => {
      ...
      const price = params?.attributes?.payinTotal?.amount / 100;

      const planName = customerplanName;

      const fetchRewardPointsSafely = async rewardId => {
        try {
          const response = await fetchRewardPoints({ rewardId });
          return response;
        } catch (error) {
          console.error(`Error fetching reward points for ${rewardId}:`, error);
          return null;
        }
      };

      const planNames = ['Free-Plan', 'business-member', 'business-enterprise'];
      const priceRanges = [
        { range: [1, 20], idSuffix: '1-20' },
        { range: [21, 500], idSuffix: '21-500' },
        { range: [501, Infinity], idSuffix: '500+' },
      ];
      ...
      for (const planName of planNames) {
        for (const { range, idSuffix } of priceRanges) {
          const rewardId = `${planName.toLowerCase()}-pricerangetransaction-points-${idSuffix}`;
          const rewardPoints = await fetchRewardPointsSafely(rewardId);

          actionArr.push({ rewardId, idSuffix });

          priceArr.push({
            planName,
            rewardPointsRange: [rewardPoints?.rewardPoints?.[0], rewardPoints?.rewardPoints?.[1]],
            range,
          });
        }
      }
      ...
      // Determine the plan based on the dynamic plan name and price range
      let selectedPlan = null;
      for (let i = 0; i < priceArr.length; i++) {
        if (
          planName === priceArr[i].planName &&
          price >= priceArr[i].range[0] &&
          price <= priceArr[i].range[1]
        ) {
          selectedPlan = priceArr[i];
          break;
        }
      }
      ...
      if (selectedPlan) {
        ...
        if (selectedPlan.planName === 'Free-Plan') {
          if (price >= 1 && price <= 20) {
            priceRange = "1-20"
            rewardPoints = getRandomInt(0, 5);
          } else if (price >= 21 && price <= 250) {
            priceRange = "21-500"
            rewardPoints = getRandomInt(5, 11);
          } else if (price > 250 && price <= 500) {
            priceRange = "21-500"
            rewardPoints = getRandomInt(11, 15);
          } else if (price > 500) {
            priceRange = "500+"
            rewardPoints = getRandomInt(15, 500);
          }
        } else if (
          selectedPlan.planName === 'business-member' ||
          selectedPlan.planName === 'business-enterprise'
        ) {
          if (price >= 1 && price <= 20) {
            priceRange = "1-20"
            rewardPoints = getRandomInt(15, 45);
          } else if (price >= 21 && price <= 250) {
            priceRange = "21-500"
            rewardPoints = getRandomInt(30, 70);
          } else if (price > 250 && price <= 500) {
            priceRange = "21-500"
            rewardPoints = getRandomInt(70, 100);
          } else if (price > 500) {
            priceRange = "500+"
            rewardPoints = getRandomInt(50, 150);
          }
        }
        const action = actionArr.find((itm) => itm.idSuffix == priceRange)?.rewardId;

        if (transitionName === transitions?.MARK_RECEIVED && firstTx) {
          // for provider
          getRewardsPoint({
            planName: providerPlanName,
            type: 'add',
        ...
        console.log('No suitable plan found for the provided plan name and price.');
      }
      ...
      if (transitionName === transitions?.MARK_RECEIVED && !firstTx) {
        const res = await fetchRewardPoints({
          rewardId: `${planName.toLowerCase()}-firstTx-points`,
        });
        // for provider
        const providerRewardsPoint =
          res?.rewardPoints?.length == 1 && providerPlanName === 'business-enterprise'
            ? res?.rewardPoints?.[0]
            : getRandomPoints(res?.rewardPoints?.[0], res?.rewardPoints?.[1]);
        getRewardsPoint({
          planName: providerPlanName,
          type: 'add',
          userId: providerUserId,
          rewardsPoint: providerRewardsPoint,
          firstTx: true,
        });

        // for customer
        const customerRewardsPoint =
          res?.rewardPoints?.length == 1 && customerplanName === 'business-enterprise'
            ? res?.rewardPoints?.[0]
            : getRandomPoints(res?.rewardPoints?.[0], res?.rewardPoints?.[1]);
        getRewardsPoint({
          planName: customerplanName,
          type: 'add',
          userId: customerUserId,
          rewardsPoint: customerRewardsPoint,
          firstTx: true,
        });
      }
      ...
      } else if (transitionName === transitions?.OFFER_DECLINE_BY_PROVIDER) {
        return sdk.messages
          .send(
            {
              transactionId: txId,
              content: 'Offer declined by you',
            },
            { expand: true }
          )
          .then(res => {
...
export const sendMessage = (txId, message, config, transactionRole, metaData) => (
  dispatch,
  getState,
  sdk
) => {
  ...
  return sdk.messages
    ...
    .then(response => {
      ...
      updateTransactionMetadata({
        id: txId,
        isCustomerMessage: transactionRole === 'customer' ? true : metaData?.isProviderMessage,
        isProviderMessage: transactionRole === 'provider' ? true : metaData?.isCustomerMessage,
        lastUpdatedTime: moment(new Date()).format('DD MMM,YYYY hh:mm A')
      });
...
export const fetchNextTransitions = id => (dispatch, getState, sdk) => {
  ...
  return sdk.processTransitions
    .query({ transactionId: id }, { expand: true })
    .then(res => {
