...
const Message = props => {
  const { message, formattedDate, transaction, isProviderRole } = props;
  const lastTransition = transaction?.attributes?.lastTransition || null;
  const content = message?.attributes?.content || "";
  const displayName = transaction?.customer?.attributes?.profile?.publicData?.userName;
  const providerUserFeed = transaction?.provider?.attributes?.profile?.publicData?.userName;
  const { MAKE_AN_OFFER, OFFER_ACCEPT_BY_PROVIDER, OFFER_DECLINE_BY_PROVIDER, MARK_DELIVERED, AUTO_COMPLETE, REVIEW_1_BY_PROVIDER, REVIEW_2_BY_CUSTOMER } = transitions || "";
  ...
  {[
    MAKE_AN_OFFER,
    OFFER_ACCEPT_BY_PROVIDER,
    OFFER_DECLINE_BY_PROVIDER,
    MARK_DELIVERED,
    AUTO_COMPLETE,
    REVIEW_1_BY_PROVIDER,
    REVIEW_2_BY_CUSTOMER
  ].includes(lastTransition) && isProviderRole ? (
    <p className={css.messageContent}>
      {`${displayName} sent an offer of ${message.attributes.content.split(" ")[5]} ${message.attributes.content
        .split(" ")
        .slice(6)
        .join(" ")}`}
    </p>
    ...
    REVIEW_1_BY_PROVIDER,
    REVIEW_2_BY_CUSTOMER
  ].includes(lastTransition) && !isProviderRole && content === "Offer Accepted by you" ? (
    <p className={css.messageContent}>
      {/* {`${providerUserFeed} Accepted your offer!`} */}
    
    <FormattedMessage id="TransactionPage.ActivityFeed.customerOfferAccepted" values={{ providerUserFeed: providerUserFeed }} />
    </p>
  ) : (
    content === "Offer declined by you" ? (
      <p className={css.messageContent}>
        {/* {`${providerUserFeed} did not accept your offer`} */}
        <FormattedMessage id="TransactionPage.ActivityFeed.customerOfferDeclined" values={{ providerUserFeed: providerUserFeed }} />
        </p>
    ) : (
...
const TransitionMessage = props => {
  ...
  const providerUsername = isProviderRole
    ? customer?.attributes?.profile?.publicData?.userName
    : isCustomerRole
      ? provider?.attributes?.profile?.publicData?.userName
      : null;
  ...
  const reviewLink = showReviewAsFirstLink ? (
    <InlineTextButton onClick={onOpenReviewModal}>
      <FormattedMessage id="TransactionPage.ActivityFeed.reviewLink" values={{ otherUsersName: providerUsername }} />
    </InlineTextButton>
  ...
  const message = intl.formatMessage(
    ...
    { actor, otherUsersName: providerUsername, listingTitle, reviewLink, deliveryMethod, stateStatus }
  );
...
export const ActivityFeedComponent = props => {
  ...
  const messageListItem = message => {
    ...
    const messageComponent = isOwnMessage ? (
      ...
      <Message message={message} formattedDate={formattedDate} transaction={transaction} isProviderRole={isProviderRole} />
    );
