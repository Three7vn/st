...
import {
  ...
  makeShippingLabelTransition,
  makeReturnShippingLabelTransition,
} from './TransactionPage.duck';
import css from './TransactionPage.module.css';
import { transitions } from '../../transactions/transactionProcessPurchase.js';
import { fetchRewardPoints, getRewardsPoint, updateRewardHistoryPoints } from '../../util/api.js';
import { disputeOrderEmailToAdmin } from '../../util/api.js';
...
const onDisputeOrder = (
  ...
  onTransition(currentTransactionId, transitionName, params)
    .then(response => {
      ...
      disputeOrderEmailToAdmin({
        transactionId: currentTransactionId?.uuid,
        textBox: disputeReason,
...
export const TransactionPageComponent = props => {
  ...
  const {
    ...
    fetchLineItemsError,
    onMakeShippingLabelTransition,
    onMakeReturnShippingLabelTransition,
  } = props;
  const planName =
  ...
  const { selectedRate, cartItems, isVerify, buyerAddress } =
    (!!transaction?.id && transaction.attributes.protectedData) || {};
  const { itemAddress, selectedCarrier, height, width, weight, length } =
    (!!listing?.id && listing.attributes.publicData) || {};
  const totalWeight =
    cartItems?.length > 0
      ? cartItems.reduce((accumulator, obj) => accumulator + obj.weight, 0)
      : weight;
  const { address_line_1, city, postalCode, state } = itemAddress || {};
  const { userName: sellerUserName, email: sellerEmail } =
  ...
  const customerEmail = customer?.attributes?.profile?.publicData?.email || '';
  ...
  const onSubmitReview = values => {
    ...
    onSendReview(transaction, transitionOptions, params, config)
      .then(r => {
        const userId = r?.data?.included[0]?.id?.uuid || '';
        if (userCreatedAt) {
          // Get the current Unix timestamp in seconds
          const currentDate = Math.floor(Date.now() / 1000);
          if (
            currentDate - userCreatedAt <= 7 * 24 * 60 * 60 &&
            rating >= 3 &&
            currentUser?.id?.uuid === userId
          ) {
            fetchRewardPoints({ rewardId: `${planName.toLowerCase()}-positivereview-points` }).then(
              res => {
                const rewardPoints = res?.rewardPoints?.length == 1 && res?.rewardPoints?.[0];
                const action = `${planName.toLowerCase()}-positivereview-points`;
                getRewardsPoint({
                  planName,
                  type: 'add',
                  userId: userId,
  ...
  const createShippingLabel = () => {
    const transitionParams = {
      ...
      carrier_account: selectedCarrier?.carrier_object_id,
      height,
      width,
      weight: totalWeight,
      length,
    ...
    onMakeShippingLabelTransition(transitionParams);
  };
  ...
  const createReturnShippingLabel = () => {
    ...
    onMakeReturnShippingLabelTransition(transitionParams);
  };
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onSendMessage: (txId, message, config, transactionRole, metaData) =>
      dispatch(sendMessage(txId, message, config, transactionRole, metaData)),
    ...
    onMakeShippingLabelTransition: transitionParams =>
      dispatch(makeShippingLabelTransition(transitionParams)),
    onMakeReturnShippingLabelTransition: transitionParams =>
      dispatch(makeReturnShippingLabelTransition(transitionParams)),
  };
};
