...
export const signup = params => (dispatch, getState, sdk) => {
  ...
  const { email, password, firstName, lastName, phoneNumber, userName,userType, ...rest } = params;
  ...
  const userData = {
    ...
    firstName,
    lastName,
    protectedData: { phoneNumber, ...rest },
    publicData: { userName, userType, fullName, email,isUserAdmin : false, featuredSeller: false },
  };

  const createUserParams = isEmpty(rest) ? userData : userData;
